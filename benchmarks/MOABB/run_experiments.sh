...
run_experiment() {
  ...
  for target_subject_idx in $(seq 0 1 $(( nsbj - 1 ))); do
    ...
    python3 train.py $hparams --seed=$seed --data_folder=$data_folder --cached_data_folder=$cached_data_folder --output_folder=$output_folder_exp\
...
for i in $(seq 0 1 $(( nruns - 1 ))); do
  ...
  # Store the results
  python3 utils/parse_results.py $output_folder_exp $metric_file $eval_metric | tee -a  $output_folder/$run_name\_results.txt
...
python3 utils/aggregate_results.py $output_folder $eval_metric | tee -a  $output_folder/aggregated_performance.txt
